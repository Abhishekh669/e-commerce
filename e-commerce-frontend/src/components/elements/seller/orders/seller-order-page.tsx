...
import { acceptOrder, deleteOrder, finishOrder } from "@/lib/actions/order/seller/seller-order-actions"
...
function SellerOrderPage() {
  ...
  const { data: ordersData, isLoading, error, refetch } = useGetSellerOrders()
  console.log("this is seller order data ; ", ordersData?.data.orders)

  // Accept order mutation
  const acceptOrderMutation = async (orderId: string) => {
    try {
      ...
      if (response.success) {
        ...
        toast.error(response.error as string || "Failed to accept order")
      }
    } catch (error) {
    ...
    const finishedOrderMutation = async (orderId: string) => {
    try {
  ...
  const deleteOrderMutation = async (orderId: string) => {
    try {
      ...
      if (response.success) {
        ...
        toast.error(response.error as string || "Failed to delete order")
      }
    } catch (error) {
                  ...
                  <div className="w-full lg:w-64">
                    ...
                    <Select value={statusFilter} onValueChange={setStatusFilter}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="paidandprocessing">Paid & Processing</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                </div>
                ...
                <CardContent>
                  ...
                  <p className="text-gray-500 mb-6 max-w-md mx-auto">
                    {searchTerm || statusFilter !== "all"
                      ...
                      : "You haven't received any orders yet. Your orders will appear here when customers purchase your products."}
                  </p>
                  {searchTerm || statusFilter !== "all" ? (
                    <Button
                      onClick={() => {
                            ...
                            <div className="flex items-center gap-3">
                               {
                                order.status === "paid and processing" && (
                                  <Button variant={"outline"} size={"sm"} onClick={()=>handleFinishOrder(order.id)} className="hover:bg-green-50 hover:border-green-300 text-green-600"
                                  >
                                    Delivered
                                  </Button>
                                )
